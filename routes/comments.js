...
const comments = require("../models/comments_neeraj_schema");
...
router.post("/reply/:comment_id/:user_id", async(req, res) => {
    ...
    try {
        ...
        const pushReplies = await comments.updateOne({ _id: req.params.comment_id }, { $push: { nestedcomments: data } }, { new: true })
            .lean()
            .exec();
...
router.delete("/reply/:comment_id/:nestedcomment_id", async(req, res) => {
    try {
        ...
        const removeNestedcomment = await comments.updateOne({ _id: req.params.comment_id }, { $pull: { nestedcomments: { _id: req.params.nestedcomment_id } } }, { new: true })
            .lean()
            .exec();
...
router.put("/reply/:comment_id/:nestedcomment_id", async(req, res) => {
    try {
        ...
        const updateNestedcomment = await comments.updateOne({ _id: req.params.comment_id, "nestedcomments._id": req.params.nestedcomment_id }, { $set: { "nestedcomments.$.comments": req.body.comments } }, { new: true })
            .lean()
            .exec();
